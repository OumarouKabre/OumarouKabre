# Bonjour, je suis Oumarou Kabré 👋

<div align="center">
  
  [![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=2E86C1&center=true&vCenter=true&width=600&lines=Ing%C3%A9nieur+Statisticien+%C3%89conomiste;Data+Scientist+%7C+Machine+Learning+Engineer;Actuariat+%7C+Finance+Quantitative;Analyse+de+donn%C3%A9es+%7C+IA+Appliqu%C3%A9e)](https://git.io/typing-svg)

</div>

---

## 🧑‍💻 À propos de moi

🎓 **Ingénieur Statisticien Économiste** en 2e année à l'**ISSEA** (Yaoundé, Cameroun)  
🏫 Diplômé **Ingénieur en Data Analyse** — **ISSP**, Ouagadougou, Burkina Faso  
📊 Passionné par la **Data Science**, le **Machine Learning**, le **Deep Learning** et l'**Actuariat**  
🔬 Auteur d'un mémoire sur la **modélisation du risque fiscal des PME** (Régression logistique — DGI Burkina Faso)  
📍 Ouagadougou, Burkina Faso  
📬 kabreoumarou95@gmail.com  

---

## 🛠️ Stack Technique

### Langages & Outils
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=for-the-badge&logo=r&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![STATA](https://img.shields.io/badge/STATA-1A5276?style=for-the-badge&logoColor=white)
![SPSS](https://img.shields.io/badge/SPSS-052FAD?style=for-the-badge&logo=ibm&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-FF6F00?style=for-the-badge&logoColor=white)

### Machine Learning & Deep Learning
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-FF6600?style=for-the-badge&logoColor=white)

### Data & Visualisation
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge&logoColor=white)

### Bases de données & DevOps
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

---

## 📂 Projets Principaux

### 🏦 Modèle de Prévision du Risque Fiscal des PME
> *Mémoire de fin d'études — Direction Générale des Impôts, Burkina Faso*

Mise en place d'un modèle de prévision des risques de **non-conformité à la TVA** parmi les PME du Burkina Faso.

- **Méthode** : Régression logistique binaire
- **Variables** : redressements fiscaux, fréquence des déclarations, statut juridique, localisation
- **Résultats** : identification des profils à risque, recommandations pour le ciblage d'audits
- **Outils** : `R` `Python` `STATA` `Excel`

---

### 🩺 Détection du Diabète par Machine Learning
> *Projet académique — ISSP*

Classification supervisée pour la détection précoce du diabète sur données médicales (UCI Diabetes Dataset).

- **Modèles** : Random Forest, SVM, XGBoost
- **Évaluation** : AUC > 85%, matrice de confusion, courbe ROC
- **Outils** : `Python` `Scikit-learn` `Pandas` `Seaborn`

---

### 💳 Scoring de Crédit
> *Projet académique — ISSP*

Modélisation du risque de défaut de paiement des emprunteurs.

- **Méthodes** : Régression logistique, Gradient Boosting
- **Interprétabilité** : SHAP values
- **Outils** : `Python` `Scikit-learn` `Matplotlib`

---

### 👁️ Computer Vision — Détection d'objets
> *Projet académique — ISSP*

Implémentation de réseaux de neurones convolutifs (CNN) pour la détection d'objets.

- **Architecture** : CNN personnalisé
- **Outils** : `Python` `TensorFlow/Keras` `OpenCV`

---

### 📊 Analyse de Survie
> *Projet académique — ISSP*

Étude de la durée de vie de patients à l'aide de méthodes de survie.

- **Méthodes** : Estimateur de Kaplan-Meier, Modèle de Cox
- **Outils** : `R` `survival` `ggplot2`

---

## 📈 GitHub Stats

<div align="center">

![Oumarou's GitHub stats](https://github-readme-stats.vercel.app/api?username=OumarouKabre&show_icons=true&theme=tokyonight&hide_border=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=OumarouKabre&layout=compact&theme=tokyonight&hide_border=true)

</div>

---

## 🎓 Formation

| Période | Diplôme | Institution |
|---------|---------|-------------|
| 2024 – 2026 | Ingénieur Statisticien Économiste *(en cours)* | ISSEA — Yaoundé, Cameroun |
| 2021 – 2024 | Ingénieur en Data Analyse | ISSP — Ouagadougou, Burkina Faso |
| 2020 – 2021 | Classes Préparatoires (CPGE) | École Polytechnique de Ouagadougou |
| 2017 – 2020 | Baccalauréat Scientifique | Lycée Mixte de Gounghin |

---

## 🌍 Langues

🇫🇷 **Français** — Langue maternelle  
🇬🇧 **Anglais** — Niveau intermédiaire (lecture technique)

---

## 📫 Me contacter

[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:kabreoumarou95@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/oumarou-kabre)

---

<div align="center">
  <i>"Les données sont la matière première du 21e siècle. La statistique est l'art de les transformer en connaissance."</i>
</div>
